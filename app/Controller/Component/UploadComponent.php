<?php
	...
	class UploadComponent extends Object {
		...
		public function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
			...
			if (isset($file) && is_array($file) && !$this->uploadError($file['error'])) {
		...
		function error ($message) {
			if (!is_array($this->errors["error"])) $this->errors["error"] = array();
			$this->errors["error"] = $message;
		}	
		
        function uploadType( $uploadType, $file ) {
       	    $srcImg = null;
		...
		function image ($file, $type, $size, $output = NULL, $quality = NULL) {
        	...
        	$srcImg = $this->uploadType( $uploadType, $file['tmp_name'] ); 
		...
		function newName ($file) {
			return date("Ymd") . time() . "." . $this->ext($file);
		}
		
        function crop($source, $targetSize = array("w","h"), $size, $position, $quality = 100) {
            if(fileExistsInPath($source)) {
                $uploadSize = getimagesize($source);
    			$uploadWidth  = $uploadSize[0];
    			$uploadHeight = $uploadSize[1];
    			$uploadType = $uploadSize[2];
    			
            	$srcImg = $this->uploadType( $uploadType, $source ); 
            	$dstR = ImageCreateTrueColor( $targetSize["w"], $targetSize["h"]);
    
            	imagecopyresampled($dstR, $srcImg,0,0,$position['x'],$position['y'],
            	$targetSize["w"],$targetSize["h"],$size["w"],$size["h"]);
                $renamedFile = $this->renameFile( $source  );
                
                 
                $write =  imagejpeg($dstR, $renamedFile["path"],$quality);
                if ($write) {
    				return $renamedFile["newfile"];
    			} else {
        ...
        function renameFile ( $source ) {
			$parts = pathinfo($source);
            $dirName = 	$parts["dirname"];
            $baseName = $parts["basename"];
            $filename = $parts["filename"];
            $ext = $parts["extension"];
            $newFile = $filename . "_cropped". ".$ext";
            $arg["newfile"] = $newFile;
            $arg["path"] = $dirName . DS .$newFile; 
            return $arg;
		}
	   ...
	   private function uploadError ($errorobj) {
			$error = false;
			switch ($errorobj) {
